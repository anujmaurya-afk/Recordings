...
async def list_jobs(
    ...
    jobs = await js.list_recent_jobs(limit=limit)
...
async def create_job_from_urls(
    ...
    # Run synchronously — serverless (Vercel) functions don't reliably keep
    # running background tasks after the response is sent.
    await run_conversion_job(job_id, payload.provider)
...
async def retry_failed(
    ...
    await run_conversion_job(job_id, provider)
...
async def download_errors_csv(job_id: str):
